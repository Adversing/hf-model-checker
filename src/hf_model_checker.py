...
with open('quant_multipliers.json', 'r') as file:
    QUANT_MULTIPLIERS = json.load(file)
...
def get_best_quantization(available_ram_gb: float, available_vram_gb: float, model_size_gb: float, available_quants: list) -> str:
    max_memory = max(available_ram_gb, available_vram_gb)
    
    filtered_quants = {k: v for k, v in QUANT_MULTIPLIERS.items() if k in available_quants}
    
    suitable_quants = []
    for quant, multiplier in filtered_quants.items():
        required_memory = model_size_gb * (multiplier + 0.1) 
...
def estimate_ram_requirement(file_name: str, file_size_bytes: int) -> float:
    ...
    base_ram = size_gb * overhead_multiplier
    attention_overhead = size_gb * 0.1
    return base_ram + attention_overhead

def get_performance_label(estimated_ram_needed: float, user_ram_gb: float, vram_gb: float = 0) -> str:
...
def group_split_files(files):
    ...
    for f in files:
        quant_type = None
        filename_upper = f.rfilename.upper()
        ...
        if quant_type not in grouped:
            grouped[quant_type] = {'size': 0, 'files': []}
        grouped[quant_type]['size'] += f.size
        grouped[quant_type]['files'].append(f)
    return grouped

def analyze_huggingface_url(repo_url: str):
    ...
    path_part = parts[1]
    repo_id = path_part.split("/tree/main/")[0] if "/tree/main/" in path_part else path_part.split("/blob/main/")[0] if "/blob/main/" in path_part else path_part
    ...
    try:
        ...
        console.print("[red]The model you're looking for does not exist or it's not public (access to private repositories/NSFW content is currently under development).[/red]")
        return
    ...
    with progress:
        ...
        if "/blob/main/" in path_part:
            ...
            if target_file:
                ...
                estimated_ram_needed = estimate_ram_requirement(file_path, target_file.size)
                performance_label = get_performance_label(estimated_ram_needed, ram_gb, vram_gb)
        ...
        if "GGUF" in repo_id.upper():
            gguf_files = [f for f in all_files if f.rfilename.endswith('.gguf')]
            if not gguf_files:
                console.print("[red]No GGUF files found in repository[/red]")
                return

            grouped_files = group_split_files(gguf_files)
            
            for quant_type, group in grouped_files.items():
                file_size_gb = group['size'] / (1024**3)
                estimated_ram_needed = estimate_ram_requirement(quant_type, group['size'])
                performance_label = get_performance_label(estimated_ram_needed, ram_gb, vram_gb)
        ...
        if viable_quants:
            gpu_ready_quants = [(q, s, p) for q, s, p in viable_quants if "GPU-Ready" in p]
            ready_quants = [(q, s, p) for q, s, p in viable_quants if "Ready" in p and "GPU-Ready" not in p]
            slow_quants = [(q, s, p) for q, s, p in viable_quants if "Will be slow" in p]
            ...
            estimated_ram_needed = estimate_ram_requirement(repo_id, total_size_bytes)
            performance_label = get_performance_label(estimated_ram_needed, ram_gb, vram_gb)
